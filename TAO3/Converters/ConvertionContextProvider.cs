using Microsoft.DotNet.Interactive;
using System;
using System.Threading.Tasks;

namespace TAO3.Converters
{
    public sealed class ConvertionContextProvider
    {
        private readonly IConverter _converter;
        private readonly Func<Task<string>> _getTextAsync;

        public ConvertionContextProvider(IConverter converter, Func<Task<string>> getTextAsync)
        {
            _converter = converter;
            _getTextAsync = getTextAsync;
        }

        public IConverterContext<TSettings> Invoke<TSettings>(string name, string settings, bool verbose, KernelInvocationContext context)
        {
            return new ConverterContext<TSettings>(_converter, name, settings, verbose, context, _getTextAsync);
        }

        public IConverterContext<object> Invoke(string name, string settings, bool verbose, KernelInvocationContext context)
        {
            return new ConverterContext<object>(_converter, name, settings, verbose, context, _getTextAsync);
        }
    }
}
